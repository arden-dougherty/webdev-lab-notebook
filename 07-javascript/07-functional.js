...
const mapNameFamily = (characters) => {
  ...
  const charactersMapped = characters.map((item) => {
...
const filterFamily = (characters, house) => {
  ...
  const charactersFiltered = characters.filter((item) => item.house == house);
...
const reduceHouses = (characters) => {
  ...
  const charactersReduced = characters.reduce((acc, curr) => {
    acc.hasOwnProperty(curr.house) ? acc[curr.house]++ : (acc[curr.house] = 1);

    return acc;
  }, {});

  return charactersReduced;
};
